#### Simulacion de parentesco patrilineal con reclutamiento, generaciones 2 a 15
...
# Y no es de la GENERACION previa, es del CEMENTERIO previo
# porque al ser patri con reclutamiento las mujeres se mueven
...
for(i in 1:6)
{
  if (length(cementerios$patrilineal1[[i]]$mujer)==0){
    grupos$patrilineal1[[i]]<-sample(haplos_int,1)
} else {
      grupos$patrilineal1[[i]]<-cementerios$patrilineal1[[i]]$mujer
}
}
