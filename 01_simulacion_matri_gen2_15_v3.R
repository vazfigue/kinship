#### Simulacion de parentesco matrilineal, generaciones 2 a 15
...
for(i in 1:6)
{
  if (length(generacion$matrilineal[[i]]$mujer)==0){
    grupos$matrilineal[[i]]<-sample(haplos_int,1)
} else {
      grupos$matrilineal[[i]]<-generacion$matrilineal[[i]]$mujer
}
}
